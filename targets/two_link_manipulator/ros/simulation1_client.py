...
class Simulation1Client:
    ...
    def doneCallback(self, state, result):
        ...
        t = np.array(result.t)
        q = np.array(result.q).reshape((N,2))
        dq = np.array(result.dq).reshape((N,2))
        r = np.array(result.r).reshape((N,2))
        e = np.array(result.e).reshape((N,2))
        de = np.array(result.de).reshape((N,2))
        v = np.array(result.v).reshape((N,2))
        tau = np.array(result.tau).reshape((N,2))
        df = pd.DataFrame()
        df['itr'] = itr
        df['t'] = t
        for i in range(2):

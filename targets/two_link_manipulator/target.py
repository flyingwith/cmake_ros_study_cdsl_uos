...
class Target:
    ...
    def simulation1_run(self, arguments, keywords):
        ...
        i += 1
        self.q = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.dq = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.r = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.e = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.de = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.v = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2
        self.tau = np.squeeze(self.df.iloc[:,i:i+2].to_numpy())
        i += 2